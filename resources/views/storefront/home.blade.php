...
@section('content')
                    ...
                    @forelse($middleBanners as $banner)
                        <div class="carousel-item min-w-full h-full relative sf-skeleton">
                            <img class="w-full h-full object-cover sf-img-loading" src="{{ $banner->mobileImageUrl($locale) }}"
                                srcset="{{ $banner->mobileImageUrl($locale) }} 768w, {{ $banner->imageUrl($locale) }} 1440w"
                                sizes="100vw" alt="{{ $banner->{"title_$locale"} }}" width="1440" height="720"
    ...
    ? ($featuredSubcategories->count() < 10 ? $featuredSubcategories->merge($featuredSubcategories) :
        ...
        <div class="overflow-hidden py-3 sf-lazy-section">
            <div class="{{ $locale === 'ar' ? 'animate-marquee-rtl' : 'animate-marquee' }} flex w-max items-center gap-3 md:gap-4 py-2"
                ...
                @php
                $subImage = $sub->image ? (str_starts_with($sub->image, 'http') ? $sub->image :
                \Illuminate\Support\Facades\Storage::url($sub->image)) : asset('meacash-logo-64.webp');
                ...
                <div onclick="window.openSubcategoryModal('{{ $sub->slug }}')"
                    ...
                    <div class="sf-brand-card-icon-shell mb-2 flex h-10 w-10 items-center justify-center rounded-2xl">
                        <img src="{{ $subImage }}" alt="{{ $subName }}" loading="lazy" width="32" height="32"
                            decoding="async"
        ...
        <div class="relative z-10 max-w-[1440px] mx-auto"> {{-- Reopen max-w-[1440px] mx-auto --}}
                    ...
                    <div
                        ...
                        <a href="{{ route('store.home') }}#products-section" data-category-link="all"
                            ...
                            <div data-category-circle
                                ...
                                style="border-radius: 9999px !important;">
                                <span
                                    class="material-symbols-outlined text-3xl md:text-4xl {{ $allActive ? 'text-[#fbbf24]' : 'text-on-surface-variant group-hover:text-[#fbbf24]' }}">{{
                                    $allAssetsIcon }}</span>
                            </div>
                            <span data-category-label
                        ...
                        if (mb_strlen($icon) <= 2) $icon='category' ; if (str_contains(strtolower($cat->name_en), 'game'))
                            $icon = 'sports_esports';
                            ...
                            <a href="{{ route('store.home', ['category' => $cat->slug]) }}#products-section"
                                ...
                                <div data-category-circle
                                    ...
                                    @if(mb_strlen($cat->icon) <= 2 && $cat->icon)
                                        ...
                                        @else
                                        <span
                                            class="material-symbols-outlined text-3xl md:text-4xl {{ request('category') == $cat->slug ? 'text-primary-container' : 'text-on-surface-variant group-hover:text-primary-container' }}">{{
                                            $icon }}</span>
                                        @endif
                                </div>
                                <span data-category-label
            ...
            {{-- Main Product Grid --}}
            <section id="products-section" class="px-4 md:px-8 py-6 min-h-[400px] sf-lazy-section">
            ...
            @if($faqs->isNotEmpty())
                <section
                    class="px-4 md:px-8 py-24 bg-surface-container-lowest/30 relative z-10 sf-reveal-section sf-lazy-section"
                    id="faq-section">
                    <div class="max-w-4xl mx-auto">
                        ...
                        <div class="space-y-4">
                            @foreach($faqs as $faq)
                                <details
                                    class="group bg-surface-container/40 rounded-2xl border border-outline-variant/10 overflow-hidden">
                                    <summary
