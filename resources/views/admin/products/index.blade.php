...
<div class="table-wrap mt-4"><table class="admin-table"><thead><tr><th>ID</th><th>Image</th><th>Name</th><th>Type</th><th>Template</th><th>Status</th><th>Price</th><th>Actions</th></tr></thead><tbody>
@forelse($products as $product)
<tr><td>#{{ $product->id }}</td><td>@if($product->image)<img src="{{ str_starts_with($product->image, 'http') ? $product->image : \Illuminate\Support\Facades\Storage::disk($disk)->url($product->image) }}" class="h-10 w-10 rounded-lg object-cover" alt="{{ $product->name_en }}">@else <span class="text-xs text-slate-400">No image</span>@endif</td><td>{{ $product->name_en }}</td><td>{{ match($product->product_type){'custom_quantity'=>'Top Up','account_topup'=>'Account',default=>'Key'} }}</td><td>{{ $product->subcategory?->productTypeDefinition?->name ?? '-' }}</td><td><span class="status-pill {{ $product->is_active ? 'ok':'off' }}">{{ $product->is_active ? 'Active':'Disabled' }}</span></td><td>${{ number_format((float)$product->selling_price,2) }}</td><td class="flex gap-2"><a href="{{ route('admin.products.show',$product) }}" class="btn-ghost">View</a><a href="{{ route('admin.products.edit',$product) }}" class="btn-ghost">Edit</a><form method="POST" action="{{ route('admin.products.duplicate', $product) }}">@csrf<button type="submit" class="btn-ghost">Duplicate</button></form><button type="button" class="btn-danger-outline js-delete-button" data-action="{{ route('admin.products.destroy', $product) }}" data-name="{{ $product->name_en }}">Delete</button></td></tr>
@empty
