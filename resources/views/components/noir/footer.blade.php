...
<footer class="relative overflow-hidden border-t border-outline-variant/20 bg-surface-container-lowest">
    <button class="fixed bottom-24 end-5 z-50 hidden h-12 w-12 items-center justify-center rounded-full bg-primary-container text-on-primary-container shadow-[0_0_20px_rgba(0,240,255,0.4)] transition-transform hover:scale-110 md:flex md:bottom-8 md:end-8" onclick="window.scrollTo({top: 0, behavior: 'smooth'})">
    ...
    <div class="px-4 py-8 md:hidden">
        <div class="mx-auto max-w-md rounded-[1.75rem] border border-outline-variant/10 bg-surface-container-low/55 p-5 text-center">
            ...
            <a href="https://broadstark.com" target="_blank" rel="noopener noreferrer" class="mt-4 inline-flex items-center justify-center gap-2 font-label text-[9px] font-black uppercase tracking-widest text-outline transition hover:text-primary-container">
        ...
        <div class="mx-auto grid max-w-[1440px] grid-cols-1 gap-10 px-8 md:grid-cols-2 lg:grid-cols-5 lg:gap-8">
            <div class="lg:col-span-2">
                <div class="mb-6 flex items-center gap-3">
                    ...
                    <span class="inline-block text-xl md:text-3xl font-black italic tracking-tighter text-transparent bg-clip-text px-2 bg-gradient-to-r from-[#00f0ff] to-[#fe00fe]">{{ config('app.name', 'MEACASH') }}</span>
                </div>
            ...
            <div>
                ...
                <ul class="space-y-4 font-label text-xs uppercase tracking-widest text-on-surface-variant">
                    @auth
                        <li><a class="transition-colors hover:text-secondary-container" href="{{ route('store.wallet') }}">{{ __('Wallet') }}</a></li>
                        <li><a class="transition-colors hover:text-secondary-container" href="{{ route('store.orders') }}">{{ __('Orders') }}</a></li>
                    @else
                        <li><a class="transition-colors hover:text-secondary-container" href="{{ route('login') }}">{{ __('Login') }}</a></li>
                        <li><a class="transition-colors hover:text-secondary-container" href="{{ route('store.register') }}">{{ __('Create Account') }}</a></li>
                    @endauth
        ...
        <div class="mx-auto mt-16 flex max-w-[1440px] flex-col items-center justify-between gap-6 border-t border-outline-variant/10 px-8 pt-8 md:mt-20 md:flex-row">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:gap-8">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="https://broadstark.com" target="_blank" rel="noopener noreferrer" class="font-headline text-[10px] uppercase tracking-widest text-on-surface-variant transition hover:text-primary-container">
