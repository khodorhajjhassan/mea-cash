...
<div {{ $attributes->merge(['class' => 'group overflow-hidden rounded-xl border border-transparent bg-surface-container-low transition-all duration-300 hover:-translate-y-1 hover:border-primary-container/30 hover:shadow-[0_0_40px_rgba(0,240,255,0.15)] sm:rounded-2xl']) }}
    ...
    <div class="relative aspect-square overflow-hidden bg-surface-container-lowest">
        ...
        @if($model->is_featured)
            <div class="absolute top-2 {{ $locale == 'ar' ? 'left-2 sm:left-3' : 'right-2 sm:right-3' }} rounded border border-secondary-container/30 bg-secondary-container px-1.5 py-0.5 font-label text-[8px] font-black uppercase tracking-tighter text-on-secondary-container shadow-[0_0_10px_rgba(254,0,254,0.5)] sm:top-3 sm:px-2 sm:py-1 sm:text-[10px]">
                {{ $locale == 'ar' ? 'مميز' : 'HOT' }}
            </div>
        @endif

        <div class="absolute bottom-2 {{ $locale == 'ar' ? 'right-2 sm:right-3' : 'left-2 sm:left-3' }} rounded border border-primary-container/30 bg-background/60 px-1.5 py-0.5 font-label text-[8px] uppercase tracking-tighter text-primary-container backdrop-blur-md sm:bottom-3 sm:px-2 sm:py-1 sm:text-[10px]">
            {{ $locale == 'ar' ? 'فوري' : 'Instant' }}
        </div>
    </div>

    <div class="p-2 sm:p-4">
        <h3 class="mb-1 truncate font-headline text-[11px] font-bold uppercase leading-tight tracking-wide transition-colors group-hover:text-primary-container sm:text-sm">
            {{ $name }}
        </h3>
        ...
        <div class="flex items-center justify-between gap-1">
            <div class="min-w-0">
                @if($price)
                    <span class="hidden text-[10px] font-bold uppercase tracking-widest text-outline sm:block">{{ $locale == 'ar' ? 'تبدأ من' : 'From' }}</span>
                    <span class="block truncate text-sm font-black leading-none text-primary-container sm:text-lg">${{ number_format($price, 2) }}</span>
                @endif
            </div>

            <button class="rounded-lg bg-surface-container-highest p-1.5 text-secondary-container shadow-inner transition-all hover:bg-secondary-container hover:text-on-secondary active:scale-90 sm:p-2" aria-label="{{ $locale == 'ar' ? 'شراء' : 'Buy' }}">
