    ...
    <nav class="flex justify-between items-center w-full max-w-[1440px] mx-auto px-4 md:px-8 h-20">
        <div class="flex items-center gap-4 md:gap-8">
            <a href="{{ route('store.home') }}" class="flex items-center gap-3 group">
                ...
                <span class="inline-block px-2 text-xl md:text-2xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#00f0ff] to-[#fe00fe]">
        ...
        <div class="flex items-center gap-3 md:gap-6">
            ...
            <div class="flex items-center gap-2 md:gap-4 text-primary-container">
                @auth
                    <div class="relative">
                        <button id="store-notification-bell" type="button" class="relative flex h-10 w-10 items-center justify-center rounded-full border border-outline-variant/25 bg-surface-container-highest/80 text-primary-container transition hover:border-primary-container/60 hover:shadow-[0_0_18px_rgba(0,240,255,0.16)]" aria-label="{{ __('Notifications') }}">
                            <span class="material-symbols-outlined text-xl">notifications</span>
                            @if($storeUnreadCount > 0)
                                <span class="absolute -top-1 -end-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-secondary-container px-1 text-[10px] font-black text-on-secondary-container ring-2 ring-background">
                                    {{ $storeUnreadCount > 9 ? '9+' : $storeUnreadCount }}
                                </span>
                            @endif
                        </button>

                        <div id="store-notification-dropdown" class="absolute {{ app()->getLocale() == 'ar' ? 'left-0' : 'right-0' }} mt-3 hidden w-[calc(100vw-2rem)] max-w-80 overflow-hidden rounded-2xl border border-outline-variant/20 bg-surface-container-lowest/95 shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-xl">
                            <div class="flex items-center justify-between border-b border-outline-variant/10 p-4">
                                <h3 class="font-headline text-sm font-black uppercase text-on-surface">{{ __('Notifications') }}</h3>
                                @if($storeUnreadCount > 0)
                                    <form method="POST" action="{{ route('store.notifications.read-all') }}">
                                        @csrf
                                        <button type="submit" class="font-label text-[9px] font-black uppercase tracking-widest text-primary-container">{{ __('Read all') }}</button>
                                    </form>
                                @endif
                            </div>
                            ...
                            <div class="max-h-80 overflow-y-auto no-scrollbar">
                                @forelse($storeNotifications as $notification)
                                    <a href="{{ route('store.notifications.read', $notification->id) }}" class="flex gap-3 border-b border-outline-variant/8 p-4 transition hover:bg-primary-container/10 {{ $notification->read_at ? 'opacity-60' : '' }}">
                                        ...
                                        <span class="min-w-0 flex-1">
                                            <span class="block truncate font-headline text-xs font-black uppercase text-on-surface">{{ $notification->data['type'] ?? __('Notification') }}</span>
                                            <span class="mt-1 line-clamp-2 block text-xs leading-relaxed text-on-surface-variant">{{ $notification->data['message'] ?? '' }}</span>
                                            <span class="mt-1 block font-label text-[9px] uppercase tracking-widest text-outline">{{ $notification->created_at->diffForHumans() }}</span>
                                        </span>
                                    </a>
                                @empty
                                    <div class="p-8 text-center font-label text-[10px] uppercase tracking-widest text-outline">{{ __('No notifications yet') }}</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
                ...
                @auth
                    <a href="{{ route('store.dashboard') }}" class="hidden scale-95 active:opacity-80 transition-transform sm:inline-flex">
                        <span class="material-symbols-outlined" data-icon="account_circle">account_circle</span>
                    </a>
                @else
                    <a href="{{ route('login') }}" class="hidden scale-95 active:opacity-80 transition-transform sm:inline-flex">
